    ...
    for path in files.values():
        if not path.exists():
            raise FileNotFoundError(
                f"Missing file: {path}. Please run preprocess.py first."
            )
...
def check_data(X_train, X_test, y_train, y_test):
    ...
    print(f"Testing class balance  : {pd.Series(y_test).value_counts().to_dict()}")
    print(f"Train/Test columns     : {list(X_train.columns) == list(X_test.columns)}")
...
def main():
    ...
    plt.ylabel("Score")
    plt.title("SVM Threshold Analysis")
    plt.legend()
    plt.grid(alpha=0.25)
    plt.tight_layout()
    ...
    print("  - best_svm_model.joblib")
    print("  - svm_decision_threshold.joblib")
    print("  - svm_metrics.csv")
    print("  - svm_tuning_comparison.csv")
    print("  - svm_threshold_comparison.csv")
    print("  - svm_threshold_metrics.png")
